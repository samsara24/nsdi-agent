...
from .llm import INJECTION_MODES, SCORE_MODES
...
def train_command(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    evaluation = pipeline.evaluate(
        ...
        injection_mode=args.kg_injection,
        score_mode=args.llm_score_mode,
...
def add_injection_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--kg-injection", choices=INJECTION_MODES, default="layered",
        help="full replays the pre-2026-08 behaviour of injecting every KG field unconditionally; "
             "layered withholds the aggregated KG score when the KG does not cover the case",
    )
    parser.add_argument(
        "--llm-score-mode", choices=SCORE_MODES, default="llm_only",
        help="legacy blends 35%% of the KG score back into the LLM route before fusion; "
             "llm_only keeps the LLM route independent of the KG score",
    )
    parser.add_argument(
        "--insufficient-confidence-scale", type=float, default=1.0,
        help="multiplier applied to the fusion confidence when the LLM reports insufficient evidence; "
             "1.0 records the signal without acting on it",
    )
