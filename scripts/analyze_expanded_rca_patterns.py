...
def render_html(data: dict[str, Any]) -> str:
    ...
    report_title = str(data.get("report_title", "扩充 RCA 证据模式冲突分析"))
    page_heading = str(data.get("page_heading", "RCA 相似 Case 标签审核"))
    candidate_heading = str(data.get("candidate_heading", "4. 双高异标签候选"))
    review_list_heading = str(data.get("review_list_heading", "待审核：高相似但标签不同"))
    review_banner = data.get("review_banner")
    storage_key = str(data.get("storage_key", "rca-expert-annotations:20260816-expanded-pattern-conflict:v1"))
    export_filename = str(data.get("export_filename", "expert_label_annotations.json"))
    export_experiment_id = str(data.get("export_experiment_id", "20260816_expanded-pattern-conflict"))
    ...
    for pattern in data["patterns"]:
        ...
        weighted_chip = (
            f'<span class="pill">加权 {float(pattern["weighted_similarity"]):.3f}</span>'
            if pattern.get("weighted_similarity") is not None else ""
        )
        routing_note = (
            f'<p><b>路由审计：</b>{esc(pattern["routing_note"])}</p>'
            if pattern.get("routing_note") else ""
        )
        ...
        <details class="pattern" data-pattern-id="{esc(pattern['pattern_id'])}" data-priority="{esc(pattern['review_priority'])}" data-search="{esc(train_id + ' ' + test_id)}" {'open' if len(pattern_cards) < 4 else ''}>
          <summary><span class="danger">{esc(pattern['pattern_id'])}</span> · {esc(pattern['summary'])}
            <span class="priority {esc(pattern['review_priority'])}">{priority_label}</span><span class="pill">特征 {pattern['feature_similarity']:.3f}</span><span class="pill graph-pill">图 {pattern['graph_similarity']:.3f}</span>{weighted_chip}</summary>
          <div class="review-core"><p><b>为什么需要专家：</b>{esc(pattern['review_reason'])}</p>{routing_note}<p><b>最大测量差异：</b>{esc(largest_text)}</p><p><b>差异分级：</b>大 {pattern['feature_differences']['counts']['large']} · 中 {pattern['feature_differences']['counts']['medium']} · 小 {pattern['feature_differences']['counts']['small']}</p><p><b>严格完全匹配：</b>{'是' if pattern['exact_two_dimensional_match'] else '否'}；质量状态兼容={'是' if pattern['quality_compatible'] else '否'}；关键缺失冲突={esc(pattern['critical_evidence_conflicts'] or '无')}</p></div>
...
</style></head><body><main><h1>{esc(page_heading)}</h1><p class="muted">生成于 {esc(data['created_at'])} · 离线专家标注工具 · 标注自动保存在当前浏览器</p>
...
<h2>{esc(candidate_heading)}</h2>
<div class="banner">{review_banner if review_banner is not None else f"<b>审核目标：</b>共找到 <b>{summary['pattern_count']}</b> 组 S_feature 与 S_graph 双高异标签 case；为保持页面简单，默认只显示最高/高优先的 <b>{summary['review_priority_counts'].get('critical', 0) + summary['review_priority_counts'].get('high', 0)}</b> 组，其余可在筛选器中查看。"}</div>
...
<h2 id="review-list">{esc(review_list_heading)}</h2>{''.join(pattern_cards) if pattern_cards else '<section>未发现达到阈值的标签冲突模式。</section>'}
...
document.getElementById('export-annotations').addEventListener('click',()=>{{const payload={{schema_version:'rca-expert-label-review-v1',experiment_id:{json.dumps(export_experiment_id, ensure_ascii=False)},exported_at:new Date().toISOString(),annotations:Object.values(annotations)}};const blob=new Blob([JSON.stringify(payload,null,2)],{{type:'application/json'}});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download={json.dumps(export_filename, ensure_ascii=False)};document.body.appendChild(link);link.click();link.remove();URL.revokeObjectURL(url);}});
