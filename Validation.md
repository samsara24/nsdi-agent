...
### V28 未覆盖样本 LLM 仲裁可行性门

LLM不得以强制三分类方式把所有未覆盖case转为自动终裁。可新增覆盖必须来自预先冻结的盲态门，
并同时满足结构化输出、物理约束校验、候选一致性和独立校准；测试标签只能用于门冻结后的评估。

通过条件：

- missing/no-anomaly与decision_graph_gap分流，前者默认补采而不是LLM硬猜。
- LLM只能在物理候选集合内排序，不得创造证据、约束ID或候选标签。
- logical4与logical8分别报告准确率、覆盖率和校准，不共享放行阈值。
- 约束违规、无支持步骤、编造证据、量测否决或JSON协议失败均不得终裁。
- 测试复盘发现的组合门必须回到训练留一或独立验证集冻结，不能在同一测试集上直接发布。

验证证据：413条未覆盖case的冻结LLM建议命中225条（54.48%）；旧门禁放行216条、仅命中
119条（55.09%）。logical8为25/59（42.37%），missing/no-anomaly为79/150（52.67%）。
探索门29条命中24条（82.76%），但95% Wilson下界仅65.45%且由测试复盘发现。结论为：
受限仲裁方向具备局部可行性，当前自动发布门保持READY，强制补全覆盖明确FAIL。

### V29 专家因果方向与 LLM 最终标签一致性门

专家规则必须描述可验证的因果链，不得把接收端症状固定映射为本端或对端根因。LLM 的最终
标签必须与其有效推理步骤、物理校验和失败状态一致；标量置信度不能覆盖致命校验结果。

通过条件：

- 远端根因只有在远端 Tx 故障或等价方向性证据存在时才可由接收症状唯一推出。
- 仅有本端 Rx/media/SerDes 对齐异常且远端 Tx 正常时，使用候选集合并降级，不自动翻转方向。
- fallback、`physical_compliance<=0`、零置信上限和 verdict/step 不一致均禁止自动终裁。
- 对旧结果的修正必须在读取测试标签前冻结；真实标签只用于之后的正确性和问题类型统计。
- 每个修正 case 保存旧专家结果、新因果规则、候选集合、原 LLM verdict、步骤汇总 verdict、
  修正动作与标签感知复盘。

验证证据：训练集对齐接收链签名共18条，本端/对端/fiber为5/10/3，证明固定方向规则不可用。
新版对413条未覆盖样本识别80条致命无效旧终裁、16条步骤/最终标签冲突和21条“物理候选
命中真实标签但旧最终标签错误”case。结构门禁和逐case审计为PASS；修正后的自动终裁仅
39/68（57.35%），自动扩覆盖门保持READY，不得替代v5正式基线。

### V30 非空分析结论与可选 Host SNR 门

面向报告和人工复核的每条case必须给出L1/L2/fiber三选一的最佳分析结论；该字段不得与
自动终裁资格混用。Host SNR是稀疏可选量测，只能提供正向增强，缺失不得造成惩罚。

通过条件：

- 全部484条测试case的`analysis_verdict`非空且只取L1/L2/fiber。
- `analysis_verdict`、`analysis_confidence_tier`与`corrected_action`分别表达最佳判断、把握程度
  和是否可自动执行；human_review/request_evidence仍可携带非空分析标签。
- 活动Prompt载荷、诊断完整度和补采清单排除缺失host_snr；原始缺失状态仍保留用于审计。
- 有效host_snr异常只能增强已有本端电口候选，不得单独支撑最终标签。
- 单侧接收症状只生成三段因果候选域；无独立Tx、接收链或双向介质证据时不得作为高置信终裁。
- 两个测试来源分别报告accuracy，完整测试同时报告Macro-F1、分类指标和混淆矩阵。

验证证据：v3报告包含484个独立case页面且所有分析标签非空；盲态文件不含真实标签字段，
关键缺失清单中host_snr命中数为0。完整测试288/484（59.50%），all_data 255/417（61.15%），
logical8 33/67（49.25%），Macro-F1 39.39%。非空输出结构门为PASS；强制全覆盖精度门为FAIL，
高置信自动终裁门保持READY，v5的57/71（80.28%）仍为安全基线。
