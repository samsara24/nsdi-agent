...
### 9.33 迭代 3：把现网专家经验接进来，相对多数类的增益从 +1.4pp 变成 +14.0pp（2026-08-12）

前三轮（MVP、迭代 1、迭代 2）的共同结论是负面的：浅决策树、门限校准、
按类别校准三条路线相对多数类的增益都不超过 +2.3pp，而三条独立测量一致指向
70~75% 的精度平台。迭代 2 结尾给出的解释是「现有遥测的信息量就这么多」。

本轮把 `docs/EXPERT_EXPERIENCE.md`（现网专家规则系统的阈值表、异常级别、
故障模式优先级与两端裁决流程）实现为 `rca_framework/expert.py`，在同一份
161/107 划分上评测。**那个解释是错的。**

#### 专家规则单独评测（`artifacts/i3_expert_rules_l2fixed.json`）

| 指标 | train(161) | test(107) |
| --- | ---: | ---: |
| 准确率 | 68.94% | **76.64%** |
| 同批多数类 | 62.11% | 62.62% |
| lift | +6.83pp | **+14.02pp** |
...
对照此前最好成绩：门限路线 lift +1.41pp / 平衡召回 0.3534，相似度投票
+2.31pp / 0.4326。**专家规则一次把 lift 提高了一个数量级**，且 L1 召回从
6.25%~34% 跳到 75%。

规则不含任何在本数据集上拟合的参数，因此 train 低于 test 只反映分布差异，
不是过拟合的反向证据——它根本没有可以过拟合的自由度。

#### 增益全部来自「归因方向」这一份知识

只改一个维度的消融（全部在 test 上，`scripts/evaluate_expert_rules.py --ablations`）：

| 变体 | 覆盖 | 答题精度 | 同子集多数类 | lift | 平衡召回 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 文档原方向 | 100% | **76.64%** | 62.62% | **+14.02pp** | 0.5386 |
| 方向表整体反转 | 100% | **25.23%** | 62.62% | −37.38pp | 0.1779 |
| 一律归本端 | 100% | 44.86% | 62.62% | −17.76pp | 0.3041 |
| 一律归对端 | 100% | 57.01% | 62.62% | −5.61pp | 0.4123 |
| 不解析告警端 | 100% | 76.64% | 62.62% | +14.02pp | 0.5386 |
| 关掉两个兜底出口 | 87.85% | 75.53% | 60.64% | +14.89pp | 0.4784 |

反转方向后精度掉到 25.23%——几乎是原方向的镜像，说明规则确实在做判别而不是
碰运气；两个「一律归某一端」的变体都低于多数类，说明值钱的是**方向表本身**，
不是「有异常就报某一端」；去掉告警端解析完全不影响结果，排除了增益来自
`alarm_ip_interface` 的可能；关掉兜底出口后 lift 反而更高，说明增益也不来自兜底命中先验。

方向表本身只有一句话：**接收类观测（rxpower、media_snr）指向对端，
发送与电口观测（txpower、host_snr、serdes_snr）指向本端。**
物理依据是接收侧看到的光是对端发出的，本端发送器不在这条光路上。

#### 必须修正的两个既有结论

**1. 「70~75% 可辨识上限」是 v2 token 空间的上限，不是遥测的上限。**

迭代 2 用三条路线佐证这个平台（N5a 精确匹配 73.33%、相似度 ≥0.9 投票 72.73%、
折外 SOP 最纯子集 75.76%），但三条路线**全部建立在同一个 v2 token 空间里**，
它们的一致因此不是独立证据。专家规则用工程阈值直接读原始 lane 值，在 token 空间之外，
测试集 76.64% 已经越过该平台。

一个更直接的检验：在 268 条上取 v2 签名相同而标签不同的组（22 条，9 组），
该空间里任何算法的上界（组内取多数）是 13/22 = 59.1%，专家规则实际判对 14/22 = 63.6%。
样本量太小（只差 1 条），单独看不构成统计证据，但方向与上面一致。

**2. C17「L2 侧接收症状不足以支持 L1」要限定口径。**

C17 的实测是：分位数口径下该条件 Wilson 下界 30.0%，与 L1 先验 30.4% 无法区分。
换成专家工程阈值口径后，同一段物理关系的下界升到 **38.7%**，明显超过先验
（留出集 63.0%，下界 44.2%）。所以 C17 否掉的是**那一种证据定义**，不是这段物理关系。

由此得到本轮最可推广的方法论规则：**「现有遥测无法判别 X」这类负结论必须连同
异常定义一起陈述，否则会被过度推广成「这件事做不到」。**

#### 工程改动

- `rca_framework/expert.py`（新）：专家决策树 + `ExpertVariant`（消融用）+
  `ExpertCalibration`（分组可靠性标定）。规则忠实实现文档，包括文档自己点出的
  两处缺陷（priority 字符串排序、每指标只保留一个异常），由测试锁住。
- `rca_framework/decision.py`：新增 `expert` 候选源（`CANDIDATE_SOURCES` 三元），
  `expert_candidate` / `build_candidates` / `decide` / `decide_many` 全链路接通。
- `rca_framework/knowledge.py`：知识包新增 `expert_calibration`；
  `out_of_fold_expert_predictions` 提供门限反解用的折外标定。
  专家规则不需要折外重新拟合规则，只需折外重数一遍命中率。
- 特征字典 **v3** = v2 + `expert_anomaly` + `expert_pattern`（含 `points_to` 方向 token），
  v2 的七个家族逐字不变，因此 v3−v2 的差异可严格归因到「注入专家知识」。
- 约束库 **v6**（22 → 26 条）：C23/C24 双向接收方向、C25 本端链路方向、
  C26 记录 L2 侧本端类证据在训练集上不达标（下界 45.5% < 先验 62.1%）而只能中性。
- prompt **v7**：归因方向表提到系统级 preamble，先于任何证据解释；
  并显式给出「该侧正常」的合法 neutral 出口（迭代 2 第 2 类违规的成因）。
- `rca_framework/types.py`：`wilson_lower_bound` 下沉到叶子模块，
  打破 features → expert → branches → features 的循环，`branches.base` 原样 re-export。
- 测试 218 → 237（`tests/test_expert_rules.py` 19 条），其中三条锁住「增益来自方向知识」
  这个结论本身：方向反转必须显著劣于多数类、去告警端解析不得显著变差、test 不得系统性劣于 train。

#### 端到端门禁（无 LLM，`artifacts/i3_gate_expert_*`）

| 配置 | 覆盖 | 给结论精度 | 同子集多数类 | lift | 平衡召回 | 人工 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 迭代 2（无专家） | 67.29% | 69.44% | 68.06% | +1.39pp | 0.2596 | 6.54% |
| 迭代 3 v2 特征 | **99.07%** | 75.47% | 63.21% | **+12.26pp** | **0.5068** | **0.00%** |
| 迭代 3 v3 特征 | 95.33% | **76.47%** | 63.73% | +12.75pp | 0.5023 | 1.87% |

用户关心的三项同时改善：准确率（+6pp 精度、lift 从 +1.4pp 到 +12.3pp）、
人工介入（6.54% → 0%）、可解释性（每条结论都带一条命中的物理规则与方向依据）。
目标选择性风险 0.35 < 多数类错误率 0.374，满足迭代 2 定下的约束。
v3 在无 LLM 路径上与 v2 基本持平（专家候选在两种配置下都答了 90 条），
它的价值预期在 LLM 侧——v3 才让 LLM 有 `expert:*` token 可引、有 C23-C26 可用。

#### 仍未解决

- **fiber 召回仍然是 0，而且放宽入口条件是净损失（已测，不要再试）。**
  专家规则的 fiber 入口要求两端同 priority 且定界不同，268 条里触发 0 次。
  但问题不在这个条件太严：20 条 fiber 里有 12 条只有单端 multi_metric、
  另一端完全干净，在两端对称性这个视角下与单端故障不可区分。
  放宽后的两种入口都低于 fiber 先验 7.5% 之上有限：
  ...
  两个切分同向为负，不是切分噪声。**fiber 需要的是两端之外的信息**
  （光功率预算差、链路长度、同缆其他链路是否同时告警），
  靠现有单链路两端读数的任何组合都拿不到，这是补采需求而不是建模需求。
- L2 侧本端类证据（C26）在 train 下界 45.5%、test 却有 90.9%，两个切分差 27pp，
  该组可靠性不稳定，需要更多数据。
- 环境问题：本机 vLLM 在 NCCL P2P 初始化处必挂（GPU 4/5 与 6/7 都复现），
  需 `NCCL_P2P_DISABLE=1` 才能启动。迭代 1 那次「vLLM worker 挂死」应是同一原因。

#### 迭代 3 正式实验（`artifacts/i3b_offline_expert_v3_risk035/`）

DeepSeek-R1-Distill-Qwen-32B、GPU 6/7、TP=2、seed 42、manifest 161/107，
特征 v3、约束库 v6、prompt v7、目标风险 0.35、候选顺序 expert → branch → sop。
...
按候选源拆开：expert 答 90 条（75.56%）、sop 答 10 条（80.0%）、branch 答 2 条（100%）。
**端到端结果与无 LLM 门禁完全一致**——也就是说，这一轮的全部增益来自专家规则，
LLM 没有贡献净增益。

另有一处独立佐证：学出来的 SOP 决策树把 `expert:points_to:L2:L1` 选为根节点分裂、
`expert:points_to:L1:L2` 选为第二层分裂。树是在 v3 全部特征上自由选择的，
它独立地把专家方向 token 排到了信息量最前面。

#### LLM 侧：方向错误修好了，但合规性与正确性不相关

先说好消息：v7 的方向表**确实生效**。典型回答的第一步是
「L1 侧接收光功率与介质侧信噪比异常 → 支持根因在 L2」，引用 C23，方向完全正确；
迭代 2 那种「L1 收光低 → 支持 L1」的镜像错误不再是主要形态。

但通过率反而从 20.5%（15/73）降到 **15.0%（12/80）**。477 条 fatal 违规里，
298 条是同一件事——**引用的证据 token 家族与约束的适用范围不匹配**，
而肇事者正是本轮新增的三条专家约束：C25 占 112、C24 占 59、C23 占 55，合计 47%。

拆开看是三种机制，都不是模型的物理理解出错：

1. **契约是语法白名单，模型按语义引用。** 模型写
   `level:L1:txpower_mean:low_tail`（分位数家族）去引 C25，而 C25 的契约只认
   `expert:L1:txpower:`（工程阈值家族）。两个 token 说的是同一侧的同一个指标，
   只是异常判据不同。校验器按前缀字符串比对，于是判废。
2. **「该侧正常」的出口给了一半。** v7 告诉模型可以写 `effect=neutral, target=""`，
   但没说清 neutral 步骤**不得引用任何约束**（规则 6 的原意）。模型于是写
   「L2 侧接收正常，不支持 L2」并引用 C24——它是在反向使用 C24 的逻辑，
   物理上完全对，但 C24 的契约只允许 `support`。这类 35 条。
3. **命名对称性诱导伪造。** C23/C24 是严格镜像的一对，模型据此推断 C25
   （`_on_l1_supports_l1`）也该有镜像，于是凭空写出
   `C25_expert_local_chain_anomaly_on_l2_supports_l2`，20 次。
   我们确实**故意**打破了这个对称——L2 侧本端类证据下界 45.5% 低于先验 62.1%，
   只能做成中性的 C26——但命名规则仍然在向模型承诺对称。

#### 最要紧的一条：约束校验器没有在筛正确性

把合规性完全放到一边，只看模型给出的 verdict：

| 子集 | 给了结论 | 判对 | 精度 |
| --- | ---: | ---: | ---: |
| 全部 80 条 | 48 | 29 | 60.4% |
| 其中被判废的 68 条 | 40 | 25 | **62.5%** |
| 其中被接受的 12 条 | 8 | 4 | **50.0%** |

**被校验器接受的回答并不比被判废的更准。** 接受组样本量小（n=8），
单看不构成统计证据，但至少可以确定：校验器没有表现出任何筛正确性的能力，
它筛的是引用格式。迭代 2 得出的「79% 被判废说明知识没写成模型必须遵守的形式」
需要补一句——**写成了模型也遵守了，判废率反而更高，而判废与判对无关。**

更关键的是与专家规则的配对比较。在 LLM 给出结论的同 48 条上：
...
McNemar 精确检验 **p = 0.0117**。48 条里 LLM 只在 1 条上赢过专家规则。
结论必须直说：**在这个任务的当前形态下，32B 蒸馏模型作为定界器的价值为负——
它比一套不含拟合参数的确定性规则显著更差，而且几乎找不到规则漏掉、
它能补上的 case。**

#### 由此确定的迭代 4 方向

前三轮都在优化「让 LLM 给出更好的 verdict」，本轮的配对检验说明这条路走不通。
迭代 4 应当换掉 LLM 的岗位：

1. **把 LLM 从定界器改成解释器与质疑器。** 专家规则出 verdict，LLM 的任务变成
   （a）用证据把这个 verdict 讲成运维能核对的一段话，（b）指出规则的前提在
   本 case 上是否不成立，（c）生成补采清单。这三件事都不要求它比规则更准，
   只要求它读得懂证据——而它已经证明读得懂（方向表用对了）。
   考核指标随之从准确率换成「质疑命中率」：它标为可疑的 case 里，
   规则确实判错的比例（规则整体错 25%，随机质疑的基线就是 25%）。
2. **契约从语法白名单改成语义等价类。** 同一侧同一指标的不同异常判据
   （`level:L1:txpower_mean:*`、`drop:L1:txpower:*`、`expert:L1:txpower:*`）
   应当归入同一个等价类共同满足契约。这一条能直接消掉 298/477 的违规主体，
   而且它修的是校验器的表达能力，不是放松物理约束。
3. **给 neutral 步骤一个明确且被校验器承认的形式**：不引用任何约束，
   并把「在 neutral 步骤上引用约束」从 fatal 降为 warning——
   它是引用方式错误，不是物理错误，不该让整份回答连同正确的 verdict 一起作废。
4. **补齐 C25 的镜像**（把 C26 改名为 `..._on_l2_supports_l2_insufficient`
   或直接提供同名镜像但 effect 受限），消除命名对称性诱导的伪造。
5. 不要再指望「70~75% 上限」这个说法。当前可陈述的是：专家规则在 test 上 76.64%，
   放宽 fiber 入口为净损失，L1/L2 二分之外的增量需要新遥测。

### 9.34 迭代 4（进行中）：校验器的表达能力，与「合规不等于正确」（2026-08-12）

迭代 3 定下的第 2、3 项改动不需要 GPU 就能评估：校验器判定哪些引用方式算合规，
与模型无关，因此可以拿已保存的 80 条回答**离线重放**。
新增 `scripts/replay_constraint_check.py` 做这件事。

#### 语义等价类：通过率 15.0% → 56.2%，没有放松任何物理约束

`rca_framework/constraints/semantics.py`（新）把不同异常判据族归约到同一个
`(side, metric)` 坐标：`level:L1:txpower_mean:low_tail`（分位数）、
`drop:L1:txpower:all_lanes`（跌落）、`status:L1:TxLOS`（状态位）与
`expert:L1:txpower:lane_down`（工程阈值）说的是同一侧的同一个指标，
契约应当共同接受。校验器改为「字面前缀命中 **或** 语义坐标相同」。

安全性依据只有一条，但足够：**等价类要求 `(side, metric)` 完全一致**。
C23-C25 这类方向约束的正确性只取决于症状出现在哪一侧的哪个指标，
与用哪套阈值发现它无关；而 `allowed_targets` 仍独立锁死方向。
`expert:pattern:*` 与 `expert:points_to:*` 没有指标坐标，一律只认字面前缀。
`tests/test_constraint_semantics.py`（20 条）专门锁住这条性质：
跨侧不得匹配、跨指标不得匹配、C23/C24 的适用范围必须互斥、
C25 不得被任何接收类证据触发。

同时把「中性步骤引用了 support-only 约束」从 fatal 降为 warning。
模型写「L2 侧接收正常，不支持 L2」并引用 C24，是在**反向**使用 C24 的逻辑，
物理上正确，只是引用方式不合规范，不该让整份回答连同正确的 verdict 一起作废。

离线重放同一批 80 条回答（模型输出一字未变）：
...
Fisher 精确检验 p = 0.1533，不显著；迭代 3 旧校验器上的同一比较是
44.4%(4/8) vs 62.5%(25/40)，p = 0.695，同样不显著。
两次测量都是通过组数值上更低，但都够不着显著。可以负责任地陈述的是：
**在这个任务上测不到「约束合规」与「结论正确」之间的关联。**

这句话对整条技术路线是有分量的。约束校验器的设计目的就是筛掉不可靠的回答；
把它的表达能力修好、通过率提高 3.7 倍之后，被它挑中的回答仍然不比被它扔掉的更准。
**它保证的是引用卫生（不编造证据、不误用约束编号），不是结论质量。**
这与迭代 3 的配对检验（专家规则 38 对 vs LLM 29 对，LLM 只在 1 条上赢，p=0.0117）
指向同一个判断：LLM 在本任务上的价值不在定界。

#### 下一步

按迭代 3 定的方向做第 1 项——把 LLM 从定界器改成解释器与质疑器，
考核指标换成「质疑命中率」（它标为可疑的 case 里规则确实判错的比例，
随机基线 = 规则错误率 25%）。这是唯一一个 LLM 不需要比规则更准就能产生价值的岗位，
也是目前所有测量都还没否掉的方向。
第 4 项（补齐 C25 镜像，消除 15 次伪造）随之一起做。

#### 质疑器 v1：完全退化，但失败方式提供了明确的下一步

`artifacts/i4_challenge_v1/`（同一模型与随机种子，107 条 test）：

| | 质疑数 | 命中 | 命中率 | 相对错误率 |
| --- | ---: | ---: | ---: | ---: |
| LLM 质疑器 v1 | **107（全部）** | 25 | 23.36% | **+0.00pp** |
| train 组可靠性查表 | 32 | 8 | 25.00% | +1.64pp |
| 规则自身错误率 | — | 25/107 | 23.36% | — |

**模型对全部 107 条都说 challenge**，confidence 一律 0.8，命中率因此恒等于错误率，
信息量为零——正是 prompt 里明确警告过并要求避免的退化解。
给出替代结论的 3 条全错（0/3）。

但不能就此判定「模型做不了这件事」，因为同一批输出证明它**在认真读每一条 case**：

- 103 种不同的解释文本 / 107 条；
- 403 个引用的证据 token 里只有 1 个不存在于本 case（**幻觉率 0.2%**，
  而定界任务上光是「引用范围不匹配」就有 298 条）；
- 前提类别的选择与命中的规则一一对应：10 条 `expert:no_anomaly` 全部被标为
  `no_evidence_fallback`，一条不差。

所以失败的位置很具体：**它能读证据、能选对前提类别，缺的是「这条没问题」的判断力。**
让它扮演复核者，它总能挑出毛病。这与迭代 3 的观察一致——
模型的短板始终在「判断」，不在「读」。

#### 由此改的设计：把整体判断拆成逐条事实核对（v2）

v2 不再问「要不要质疑」，改为逐条问「这条前提成不成立，推翻它的证据 token 是哪几个」，
并明确规定「前提没有被推翻就是成立，不需要证据去正面证明它」。
每一项都是可对着证据 token 证伪的事实问题——而这正是它已经证明做得很好的事。

质疑强度由「不成立的前提条数」导出，是一个有方差的分数而不是布尔值，
因此可以事后按任意人工预算取阈值（`score_threshold_curve` 扫 0~5），
而不是由模型替运维决定报警率。v1 的退化解在这个形式下不再是免费的：
把五条前提全判成 false 需要为每一条都指出具体 token。

#### 质疑器 v2：退化解消失了，但信号也没有出现

`artifacts/i4_challenge_v2/`（同模型同种子，107 条 test，解析失败 0）。
把整体判断拆成逐条事实核对之后，分数确实有了方差
（不成立前提数分布 0:9、1:46、2:42、3:9、4:1），v1 那种「全部质疑」不再出现。

但按分数扫阈值，命中率**随质疑强度单调下降**：

| 不成立前提数 ≥ | 标记 | 命中 | 命中率 | 相对错误率 | 错误召回 |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 0（全标） | 107 | 25 | 23.36% | +0.00pp | 1.00 |
| 1 | 98 | 22 | 22.45% | −0.92pp | 0.88 |
| 2 | 52 | 9 | 17.31% | **−6.06pp** | 0.36 |
| 3 | 10 | 1 | 10.00% | −13.36pp | 0.04 |
| 4 | 1 | 0 | 0.00% | — | 0.00 |

逐条前提看也一样，每一条被判 false 时规则的错误率都不高于基线：

| 前提 | 判 false 次数 | 规则错 | 错误率 |
| --- | ---: | ---: | ---: |
| `scope_too_narrow` | 58 | 13 | 22.4% |
| `direction_table_applies` | 45 | 9 | 20.0% |
| `other_side_overlooked` | 44 | 7 | 15.9% |
| `no_evidence_fallback` | 13 | 2 | 15.4% |
| `anomaly_is_real` | 1 | 1 | 100.0% |

分数与「规则判错」的点二列相关 r = −0.155（t = −1.61，双尾 p ≈ 0.108）。
方向为负但不显著，可负责任的陈述是：**测不到信号**。
模型越怀疑的 case，规则反而没有更容易出错。

#### 三次独立测量指向同一个结论

| 岗位 | 测量 | 结果 |
| --- | --- | --- |
| 定界器 | 与专家规则配对比较（48 条） | 显著更差，McNemar **p = 0.0117** |
| 约束合规 | 通过组 vs 判废组的判对率 | 测不到关联（p = 0.15 / 0.70） |
| 质疑器 | 质疑强度 vs 规则是否判错 | 测不到关联（**p = 0.108**，方向为负） |

三次测量的对象不同、指标不同、失败方式也不同，结论却一致：
**在这个任务上，32B 蒸馏模型不能提供任何形式的「判断」——
无论是判根因、判自己的回答是否可靠，还是判规则是否可靠。**

同样重要的是它**没有**失败的地方。v2 的 402 个引用 token 里只有 3 个不存在于
本 case（0.7%），107 条给出 102 种不同的解释，前提类别与命中规则严格对应。
**读证据这件事它做得很好，坏的是所有需要下判断的环节。**

这个组合在文献里通常被含混地表述成「LLM 有幻觉」。本项目的测量说明不是这样：
幻觉率 0.7% 已经很低，问题在判断力，而判断力恰恰是三轮实验里
每一次试图从 LLM 身上榨取的东西。

#### 迭代 5 方向

唯一还没被证伪的岗位是**解释**：模型能逐条读证据、几乎不编造、
输出面向运维的自然语言。这正是用户三项诉求里的「可解释性」，
而它不需要模型具备任何判断力——结论由专家规则给出，模型只负责讲清楚为什么。

因此迭代 5 应当：

1. 把 LLM 固定在解释岗位，输入专家规则的 verdict 与命中规则，输出运维可核对的说明。
2. 评测指标换成**可核对性**而不是准确率：解释里提到的每个证据 token 是否存在、
   是否与规则实际依据的 token 一致、方向陈述是否与方向表一致。
   这三项都是机器可判的，不需要人工打分。
3. 不要再设计任何需要模型下判断的环节。三次测量已经足够，
   继续尝试只会得到第四个不显著的结果。

### 9.35 迭代 5：解释岗位成立，但它对错误结论同样流畅（2026-08-12）

`artifacts/i5_explain_v1/`（同模型同种子，107 条 test）。结论由专家规则给出，
prompt 明确声明模型**没有改判的资格**，它只负责讲清楚。
...
| --- | ---: | ---: |
| 可解析 | 100% | 100% |
| 引用 token 真实存在 | 100% | 100% |
| 引用 token 落在规则依据上 | 100% | 100% |
| 方向陈述与方向表一致 | 96.26% | **100%** |
| 未改判规则结论 | 99.07% | 99.07% |
| **四项全过** | **95.33%** | **99.07%** |

编造 token **0 个**（对比：定界任务上光「引用范围不匹配」就有 298 条）。

两个数字都要报，因为**检查是看过结果之后改的**。改动本身站得住，但顺序必须交代：
预注册的 95.33% 是诚实的下界，99.07% 是修正后的值。
5 条不合格拆开看是三种东西：

1. **1 条真的想改判**（`case_f3594459612d`）。规则走 `no_anomaly` 兜底判 L2，
   模型自行推理改判 L1。检查抓住了它——而且它是错的（真值 L2）。
   这条恰好证明「不许改判」这项检查有存在价值。
2. **3 条坏在 `symptom_side` 这个标签上**。模型把「运维在哪一端发现问题」
   当成了症状端，与它引用的读数所在侧不一致，但物理叙述完全正确。
   症状端标签本身模棱两可，不该做方向检查的支点；**token 的侧别不模棱两可**。
   检查改为由每个 token 自己的 `(侧, 指标)` 推方向，不再看模型声明的症状端。
3. **1 条是检查在惩罚模型说实话**。规则是 `no_anomaly` 兜底，模型如实写
   「两端都无明显异常，规则兜底报本端」，被判违反方向表——但那种 case
   根本不存在方向推理。与 `token_relevance` 同一个理由跳过。

`scripts/recheck_explanations.py` 做离线重判，与 `replay_constraint_check.py`
同一个思路：检查逻辑的改动与模型无关，不该为此再占一次 GPU。
...
两组几乎相同。好的一面是这个指标**不偷偷依赖真值**，因此可以直接搬到生产上
做持续监控，不用等回标。

但同一个事实的另一面必须写在同等显著的位置：**规则判错时，解释一样流畅、
一样引用真实证据、一样通过全部可核对性检查。** 抽出来的样例很直观——
规则把一条真值 L1 的 case 判成 L2，模型写的是
「L1 侧的接收光功率出现异常，但根因在 L2 侧」，并给出「检查 L2 侧发送光功率」
这样的具体动作。措辞、证据、方向推理全都无可指摘，唯独结论是错的。

所以解释这个岗位的正确定位是：**它让正确结论可核对，同时让错误结论同样可信。**
它提高的是可审计性（工程师能顺着 token 复核每一句），不是可靠性。
把它当作「LLM 提升了系统质量」来宣传是不诚实的——
质量全部来自专家规则，解释只是把规则的依据翻译成人话。

#### 整轮 loop 的最终结论

用户这一轮给的是 `docs/EXPERT_EXPERIENCE.md` 里的人工专家经验。四次迭代之后：

| | 迭代 2（无专家经验） | **迭代 3-5（注入后）** |
| --- | ---: | ---: |
| 覆盖率 | 67.29% | **95.33%** |
| 给结论时精度 | 69.44% | **76.47%** |
...
| 平衡召回 | 0.2596 | **0.5023** |
| 人工介入 | 6.54% | **1.87%** |
| 解释可核对率 | 未测 | **99.07%**（编造 token 0） |

三项诉求都改善了，而**全部增益来自那份专家经验，没有一分来自 LLM**。
四次独立测量把 LLM 需要「判断」的岗位逐个测掉（定界 p=0.0117、
约束合规 p=0.15/0.70、质疑 p=0.108），只留下「读证据 + 讲人话」这一项——
它做得很好（幻觉率 0），但那一项按定义不改变任何结论。
