...
- 可解释 token：78 种。
- 证据图：evidence-graph-v1:124:affc399cf8706073，包含 124 个历史 case。
- learned SOP：numeric-decision-tree-v1，hash `2e84eb36c2257ea7`。
- 留一法路由：`{'N5a': 10, 'N5b': 42, 'N5c': 72}`。
- 每条训练 case 的特征、数值量测、SOP 路径、留一法路由和 Top-5 历史候选保存在 `case_audit.json`。

## 可复核结论

- 118 个 signature 中 112 个仅有 1 条支持；训练 signature 高纯度不能直接解释为可泛化准确率。
- 混合标签 signature 覆盖 2 条，不能作为 N5a 自动复用模式。
